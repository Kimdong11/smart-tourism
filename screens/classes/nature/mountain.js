...
const mountain = ({ route, navigation: { navigate } }) => {
    ...
    const getContents = async () => {
        const results = await (
            await fetch(
                "http://203.253.207.111:8080/jsmith/restful/content?type=0&page=1&hcd=A0102"
            )
        ).json();
        ...
        testData = preData.filter((item) => {
            return item.hcnt_rcmd_comt_fth !== "" && item.hcnt_rcmd_comt_thd !== "";
        });
